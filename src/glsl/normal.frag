...
void main() {
    ...
    vec2 p = src.xz;
    float dr = 0.1;
    float n0 = heightmap(p, scale, height, tNoise, tNoiseSize);
    float nx = heightmap(p + vec2(dr, 0), scale, height, tNoise, tNoiseSize);
    float nz = heightmap(p + vec2(0, dr), scale, height, tNoise, tNoiseSize);
